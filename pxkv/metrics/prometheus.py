...
def registry_to_prometheus(metrics: Dict[str, Any]) -> str:
    ...
    qpc = metrics.get("query_plan_cache", {}) or {}
    lines.append("# HELP pxkv_query_plan_cache_enabled Whether query plan caching is enabled (1=on).")
    lines.append("# TYPE pxkv_query_plan_cache_enabled gauge")
    lines.append(f"pxkv_query_plan_cache_enabled {1 if qpc.get('enabled') else 0}")
    lines.append("# HELP pxkv_query_plan_cache_entries Current query plan cache entries by kind.")
    lines.append("# TYPE pxkv_query_plan_cache_entries gauge")
    lines.append(f'pxkv_query_plan_cache_entries{{kind="redis"}} {int(qpc.get("redis_entries", 0) or 0)}')
    lines.append(f'pxkv_query_plan_cache_entries{{kind="scan"}} {int(qpc.get("scan_entries", 0) or 0)}')
    lines.append("# HELP pxkv_query_plan_cache_hits_total Query plan cache hits by kind.")
    lines.append("# TYPE pxkv_query_plan_cache_hits_total counter")
    lines.append(f'pxkv_query_plan_cache_hits_total{{kind="redis"}} {int(qpc.get("redis_hits", 0) or 0)}')
    lines.append(f'pxkv_query_plan_cache_hits_total{{kind="scan"}} {int(qpc.get("scan_hits", 0) or 0)}')
    lines.append("# HELP pxkv_query_plan_cache_misses_total Query plan cache misses by kind.")
    lines.append("# TYPE pxkv_query_plan_cache_misses_total counter")
    lines.append(f'pxkv_query_plan_cache_misses_total{{kind="redis"}} {int(qpc.get("redis_misses", 0) or 0)}')
    lines.append(f'pxkv_query_plan_cache_misses_total{{kind="scan"}} {int(qpc.get("scan_misses", 0) or 0)}')
    lines.append("# HELP pxkv_query_plan_cache_hit_ratio Query plan cache hit ratio by kind.")
    lines.append("# TYPE pxkv_query_plan_cache_hit_ratio gauge")
    lines.append(f'pxkv_query_plan_cache_hit_ratio{{kind="redis"}} {float(qpc.get("redis_hit_ratio", 0.0) or 0.0)}')
    lines.append(f'pxkv_query_plan_cache_hit_ratio{{kind="scan"}} {float(qpc.get("scan_hit_ratio", 0.0) or 0.0)}')
